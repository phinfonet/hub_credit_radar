...
if Code.ensure_loaded?(CreditRadar.Ingestions.Tasks.IngestCriCra) do
  IO.inspect(CreditRadar.Ingestions.Tasks.IngestCriCra.__info__(:functions), label: "Exported functions")

  # Try to check function_exported?
  IO.inspect(function_exported?(CreditRadar.Ingestions.Tasks.IngestCriCra, :run, 0), label: "run/0 exported?")
  IO.inspect(function_exported?(CreditRadar.Ingestions.Tasks.IngestCriCra, :run, 1), label: "run/1 exported?")
else
