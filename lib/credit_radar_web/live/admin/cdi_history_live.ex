defmodule CreditRadarWeb.Live.Admin.CDIHistoryLive do
  use Backpex.LiveResource,
    adapter_config: [
      schema: CreditRadar.FixedIncome.CDIHistory,
      repo: CreditRadar.Repo,
      update_changeset: &CreditRadar.FixedIncome.cdi_history_changeset/3,
      create_changeset: &CreditRadar.FixedIncome.cdi_history_changeset/3
    ],
    layout: {CreditRadarWeb.Layouts, :admin}

  @impl Backpex.LiveResource
  def singular_name, do: "CDI Diário"

  @impl Backpex.LiveResource
  def plural_name, do: "Histórico CDI"
  ...
  defmodule ImportHistoryAction do
    ...
    def handle(socket, %{start_date: %Date{} = start_date}) do
      with {:ok, %{processed: processed}} <-
             FixedIncome.import_cdi_history(%{
        ...
        {:ok,
         ...
         |> put_flash(
           :info,
           "Histórico de CDI sincronizado (#{processed} registros processados)."
         )}
      else
        {:error, reason} ->
          {:error, put_base_error(socket, "Falha ao importar CDI: #{format_reason(reason)}")}
