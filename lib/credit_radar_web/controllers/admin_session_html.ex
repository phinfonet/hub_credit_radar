defmodule CreditRadarWeb.AdminSessionHTML do
  @moduledoc """
  Templates HTML para autenticação de administradores.
  """
      ...
      <div class="w-full max-w-md rounded-3xl border border-white/10 bg-white/5 p-8 shadow-2xl shadow-black/40 backdrop-blur">
        <div class="mb-10 space-y-3 text-center">
          ...
          <div>
            ...
            <p class="text-sm uppercase tracking-[0.4em] text-white/60">Admin Login</p>
          </div>
        ...
        <.form for={@form} id="admin-login-form" action={~p"/admin/login"} method="post" class="space-y-6">
          <.input field={@form[:email]} type="email" label="E-mail" placeholder="admin@exemplo.com" required />
