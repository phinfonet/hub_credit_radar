defmodule CreditRadar.Ingestions.Tasks.IngestDebenturesPlus do
  @moduledoc false
  ...
  @doc """
  Executes the Debêntures Plus ingestion pipeline.
  """
  def run(execution \\ %{}) do
    Logger.info("Starting Debêntures Plus ingestion for execution #{execution_id(execution)}")
    ...
    with {:ok, payload} <- fetch_remote_data(),
      ...
      {:error, reason} = error ->
        Logger.error("Debêntures Plus ingestion failed: #{inspect(reason)}")
        error
    end
  end
  ...
  defp fetch_remote_data do
    Client.fetch_debentures_plus_secondary()
    |> normalize_response()
  end
  ...
  def persist_operations(operations) when is_list(operations) do
    ...
    Logger.info(
      "Debêntures Plus ingestion persisted #{stats.created + stats.updated} securities " <>
